...
const WorkCover = ({ data }: WorkProps) => {
  ...
  return (
    <div
      ...
      className="border-1 my-12 border-secondary-900 bg-frosted dark:bg-white dark:border-darkblue rounded-xl px-4 transform transition-transform hover:scale-[1.02]"
    >
      ...
      <div className="flex flex-wrap md:flex my-6">
        {tech.map((e, index) => (
          <div
            key={index}
            className="px-4 bg-primary dark:bg-darkblue text-white rounded-full mx-2 my-3"
          >
            {e}
          </div>
        ))}
